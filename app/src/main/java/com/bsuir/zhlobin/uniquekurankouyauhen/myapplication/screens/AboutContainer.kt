...
fun AboutContainer(
    ...
    NavigationController(
        startDestination = Screen.Weather.screenName,
        router = externalRouter,
        screens = listOf(
            Pair(Screen.Weather.screenName, { nav, router, _ -> AboutApp(innerPadding) }),
                    Pair(Screen.Home.screenName, { _, _, params ->
                params?.getParcelable<Memory>(Memory.USER)?.let {
                    UserScreen(model = it)
                }
            }
            )
